...
![Image title](bio_cell.png)

*یک سلولِ یوکاریوت و اندامک‌های اصلی‌اش: هسته (با هستک)، میتوکندری، شبکهٔ آندوپلاسمیِ زبر، دستگاهِ گلژی و غشای سلول — همه شناور در سیتوپلاسم.*
...
![Image title](bio_neuron.png)

*ساختارِ یک نورون: **دندریت‌ها** ورودی می‌گیرند، **سوما** (جسمِ سلولی) آن‌ها را جمع می‌کند، و **آکسونِ** میلین‌دار — با **گره‌های رانویه** میانِ قطعاتِ میلین — سیگنال را به **پایانه‌های آکسونی** می‌رساند. فلش‌های سبز جهتِ هدایتِ سیگنال را نشان می‌دهند.*
...
![Image title](bio_bilayer.png)

*دولایهٔ فسفولیپیدی در آب **خودآرایی** می‌کند: سرهای آب‌دوست رو به آب (درون و بیرونِ سلول) و دم‌های آب‌گریز رو به یکدیگر، و یک هستهٔ چربِ نفوذناپذیر به ضخامتِ حدودِ ۵ نانومتر می‌سازند.*
...
![Image title](bio_ion_channel.png)

*یک کانالِ یونیِ **گزینشی** که غشا را می‌شکافد. **فیلترِ گزینش‌گر** (اکسیژن‌های قرمز) پتاسیم را پس از کندنِ پوستهٔ آبی‌اش عبور می‌دهد، اما سدیمِ آب‌پوشیده را پس می‌زند؛ فلشِ سبز شارِ خالصِ پتاسیم را نشان می‌دهد.*
...
این چهار سازوکارِ اصلیِ دریچه‌گذاری را می‌توان در یک نگاه کنار هم دید:

![Image title](bio_channel_gating.png)

*چهار شیوهٔ دریچه‌گذاریِ کانالِ یونی. **وابسته به ولتاژ:** حسگرِ باردارِ کانال (بخشِ S4) با تغییرِ ولتاژِ غشا جابه‌جا می‌شود و کانال را می‌گشاید — موتورِ پتانسیلِ عمل. **وابسته به لیگاند:** چسبیدنِ یک مولکولِ پیام‌رسان (مثلاً ناقلِ عصبی) به گیرنده، کانال را باز می‌کند — پایهٔ سیناپس. **مکانیکی:** کشش یا فشارِ غشا کانال را می‌گشاید — پایهٔ لامسه و شنوایی. **نشتی:** همیشه باز است و شارِ پیوستهٔ یون (به‌ویژه پتاسیم) را برقرار می‌کند — تنظیم‌کنندهٔ پتانسیلِ استراحت.*

### دریچه‌گذاری تصادفی است

نکته‌ای که اغلب پنهان می‌ماند این است که باز و بسته‌شدنِ یک **تک‌کانال** رویدادی **تصادفی** است، نه ساعت‌وار. یک کانالِ منفرد کاتوره‌ای میانِ باز و بسته می‌جهد، و آنچه ولتاژ (یا لیگاند) تعیین می‌کند تنها **احتمالِ** بازبودن است، نه لحظهٔ دقیقِ آن. پس چگونه از این آشوبِ تصادفی، آن منحنی‌های صاف و تکرارپذیری بیرون می‌آید که در فصل‌های بعد (مانندِ [هاجکین–هاکسلی](https://computational-neuroscience.ir/ch03/)) به کار می‌بریم؟

![Image title](bio_stochastic_channels.png)

*از تک‌کانالِ تصادفی تا رساناییِ صاف. **(a)** سه تک‌کانال که کاتوره‌ای باز/بسته می‌شوند. **(b)** میانگینِ جمعیتی روی \(N\) کانال؛ هرچه \(N\) بزرگ‌تر، این میانگین به منحنیِ **قطعیِ** \(m(t)\) نزدیک‌تر می‌شود. **(c)** دامنهٔ نسبیِ نوفه مانندِ \(1/\sqrt{N}\) کوچک می‌شود.*

پاسخ در **قانونِ اعدادِ بزرگ** است: یک تکه از غشا هزاران کانال دارد، و میانگینِ رفتارِ تصادفیِ آن‌ها منحنیِ تقریباً صافی می‌سازد که نوسانش مانندِ \(1/\sqrt{N}\) کوچک می‌شود. به همین سبب می‌توانیم رساناییِ یونی را با یک تابعِ **قطعی و پیوسته** توصیف کنیم — کاری که مدلِ هاجکین–هاکسلی می‌کند. اما در سلول‌های کوچک با کانال‌های اندک، همین **نوفهٔ کانالی** می‌تواند بر زمان‌بندیِ شلیک اثر بگذارد؛ و این پلی است به مدل‌های **تصادفیِ** نورون.
...
این‌ها پرسش‌های مفهومی‌اند؛ نخست خودتان پاسخ دهید، سپس کشوییِ «پاسخ» را باز کنید.

!!! question "تمرینِ ۱"
    **نفوذناپذیری.** به‌زبانِ خودتان توضیح دهید چرا یک یونِ باردار به‌سختی از میانهٔ دولایهٔ لیپیدی می‌گذرد، حال آنکه یک مولکولِ کوچکِ ناقطبی (مانندِ \(\text{O}_2\)) به‌آسانی عبور می‌کند.

??? success "پاسخ"
    یک یون در آب با پوسته‌ای از مولکول‌های قطبیِ آب احاطه شده و بارش را در محیطی با ثابتِ دی‌الکتریکِ بالا «راحت» نگه می‌دارد. میانهٔ دولایهٔ لیپیدی یک محیطِ **چرب و ناقطبی** با دی‌الکتریکِ بسیار پایین است؛ برای واردشدنِ یون باید پوستهٔ آبی را کنار بگذارد و بارش را در محیطی که بار را «دوست ندارد» ببرد — این هزینهٔ انرژیِ هنگفتی دارد (سدِ انرژیِ بورن). در مقابل، \(\text{O}_2\) ناقطبی است، پوستهٔ آبی ندارد و در چربی حل می‌شود، پس آزادانه عبور می‌کند. درسِ کلیدی: غشا برای یون‌ها یک سدِ **انرژیِ الکترواستاتیک** است، نه صرفاً یک دیوارِ فیزیکی — و برای همین است که یون‌ها به **کانال** نیاز دارند.

!!! question "تمرینِ ۲"
    **دریچه‌گذاری.** برای هر یک از سه نوع کانالِ دریچه‌دار (وابسته به ولتاژ، لیگاند، مکانیکی) یک مثالِ زیستی بیاورید که در آن آن نوع کانال نقشِ اصلی را دارد.

??? success "پاسخ"
    - **وابسته به ولتاژ:** کانال‌های سدیمی و پتاسیمیِ آکسون که پتانسیلِ عمل را می‌سازند (فصلِ پتانسیل عمل و مدلِ هاجکین–هاکسلی).
    - **وابسته به لیگاند:** گیرندهٔ AMPA/GABA در سیناپس که با چسبیدنِ ناقلِ عصبی باز می‌شود (فصلِ سیناپس‌ها).
    - **مکانیکی:** کانال‌های Piezo در گیرنده‌های لامسه، و کانال‌های سلول‌های موییِ گوشِ داخلی که ارتعاشِ صوت را به سیگنالِ الکتریکی بدل می‌کنند.

!!! question "تمرینِ ۳"
    **گزینش‌گری.** اگر گزینش‌گری تنها بر پایهٔ اندازهٔ منفذ بود، انتظار داشتیم کدام یون (سدیم یا پتاسیم) راحت‌تر عبور کند؟ چرا مشاهدهٔ واقعی برعکسِ این است، و این دربارهٔ سازوکارِ فیلترِ گزینش‌گر چه می‌گوید؟

??? success "پاسخ"
    اگر ملاک تنها اندازهٔ منفذ بود، **سدیمِ کوچک‌تر** باید راحت‌تر عبور کند. اما در عمل کانالِ پتاسیمی، پتاسیم را حدودِ **هزار برابر** بهتر از سدیم عبور می‌دهد. توضیح این است که گزینش‌گری بر پایهٔ **توازنِ انرژی** است، نه اندازه: فیلترِ کانالِ پتاسیمی چنان آرایشی از اکسیژن‌های کربونیل دارد که فاصله‌شان دقیقاً با یونِ پتاسیمِ آب‌زدوده جور است و جایِ پوستهٔ آبی را می‌گیرد، پس هزینهٔ آب‌زداییِ پتاسیم جبران می‌شود. سدیمِ کوچک‌تر نمی‌تواند هم‌زمان با همهٔ این اکسیژن‌ها تماس بگیرد، پس هزینهٔ آب‌زدایی‌اش جبران‌نشده می‌ماند و عبورش پرهزینه است. نتیجه: گزینش‌گری یک پدیدهٔ **انرژیِ شیمیایی** است، نه صافیِ مکانیکی.
