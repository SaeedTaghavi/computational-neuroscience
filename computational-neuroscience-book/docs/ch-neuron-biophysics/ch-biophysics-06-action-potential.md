...
![Image title](bio_ap_sequence.png)

*پتانسیلِ عمل یک **توالیِ پنج‌مرحله‌ای** از رخدادهای کانالی است. **بالا:** ردِ ولتاژ؛ از استراحت (‎۷۰− میلی‌ولت) با عبور از آستانه (‎۵۵−) به قله (‎۴۰+) می‌جهد، سپس رپلاریزه می‌شود، اندکی **فرابیش‌قطبش** می‌کند و به استراحت بازمی‌گردد. **پایین:** حالتِ کانال‌های سدیمی و پتاسیمیِ **وابسته به ولتاژ** در هر مرحله — بسته، باز یا غیرفعال. در بخش‌های بعدی این توالی را باز می‌کنیم.*
...
ریشهٔ این پدیده در آن است که کانالِ سدیمیِ وابسته به ولتاژ نه دو، بلکه **سه** حالت دارد:

![Image title](bio_na_channel_states.png)

*سه حالتِ کانالِ سدیمیِ وابسته به ولتاژ. **بسته:** در استراحت، دریچهٔ **فعال‌سازِ** (m) بسته است و سدیم نمی‌تواند بگذرد. **باز:** با دپلاریزاسیون، دریچهٔ فعال‌ساز می‌گشاید و سدیم به درون می‌ریزد. **غیرفعال:** پس از تأخیری کوتاه، **توپِ غیرفعال‌سازِ** (h) دهانهٔ کانال را از درون می‌بندد؛ کانال تا زمانی که غشا رپلاریزه نشود و این توپ بازآرا نشود، نمی‌تواند دوباره باز شود. همین حالتِ سومِ «غیرفعال» — نه صرفاً «بسته» — سرچشمهٔ دورهٔ بی‌پاسخی است.*
...
آمیزه‌ای از استدلالِ مفهومی و یک محاسبهٔ ساده. نخست خودتان پاسخ دهید.

!!! question "تمرینِ ۱"
    **همه‌یاهیچ.** چرا دامنهٔ پتانسیلِ عمل به شدتِ محرک بستگی ندارد، اما **بسامدِ** شلیک بستگی دارد؟ این را با ایدهٔ آستانه و دورهٔ بی‌پاسخی توضیح دهید.

??? success "پاسخ"
    **دامنه** را نه محرک، بلکه چرخهٔ **بازخوردِ مثبتِ** کانال‌های سدیمی تعیین می‌کند: هر محرکِ فراآستانه همان انفجارِ کاملِ سدیم را راه می‌اندازد و \(V\) را تا نزدیکِ \(E_{\text{Na}}\) می‌برد، فارغ از اینکه محرک چقدر از آستانه بیشتر بوده — پس دامنه تقریباً ثابت است (**همه‌یاهیچ**). اما **بسامد** فرق می‌کند: پس از هر اسپایک، غشا باید از دورهٔ **بی‌پاسخی** بیرون بیاید و دوباره به آستانه برسد. محرکِ قوی‌تر سریع‌تر غشا را به آستانه می‌رساند، پس فاصلهٔ میانِ اسپایک‌ها کوتاه‌تر و بسامد بالاتر می‌شود. خلاصه: شدتِ محرک در **نرخِ** اسپایک‌ها کد می‌شود، نه در اندازه‌شان.

!!! question "تمرینِ ۲"
    **دورهٔ بی‌پاسخی و بیشینهٔ بسامد.** اگر دورهٔ بی‌پاسخیِ مطلق حدودِ ۲ میلی‌ثانیه باشد، بیشینهٔ بسامدِ شلیک تقریباً چقدر است؟

??? success "پاسخ"
    دورهٔ بی‌پاسخیِ مطلق کمینه‌فاصلهٔ ممکن میانِ دو اسپایک را تعیین می‌کند. اگر این فاصله \(2\) میلی‌ثانیه باشد، بیشینهٔ بسامد برابرِ

    \[
    f_{\max} \approx \frac{1}{2\ \text{ms}} = \frac{1}{0.002\ \text{s}} = 500\ \text{Hz}.
    \]

    در عمل بیشترِ نورون‌ها زیرِ این حد شلیک می‌کنند چون دورهٔ بی‌پاسخیِ **نسبی** هم پس از آن ادامه دارد. همین سقف، یکی از دلایلی است که مغز اطلاعاتِ سریع را با **جمعیت**‌های نورونی کد می‌کند، نه با یک نورونِ تنها.

!!! question "تمرینِ ۳"
    **میلین.** با استفاده از رابطهٔ ثابتِ طولِ \(\lambda=\sqrt{R_m/R_a}\) از فصلِ کابل، توضیح دهید چرا بالابردنِ \(R_m\) توسطِ میلین به هدایتِ سریع‌تر کمک می‌کند.

??? success "پاسخ"
    میلین با عایق‌کردنِ غشا، مقاومتِ غشاییِ \(R_m\) را بسیار بالا می‌برد (و ظرفیتِ خازنی را پایین می‌آورد). چون \(\lambda=\sqrt{R_m/R_a}\)، بزرگ‌شدنِ \(R_m\) **ثابتِ طول را بزرگ‌تر** می‌کند؛ یعنی جریانِ محلیِ هر گرهٔ رانویه بدونِ افتِ چندان، دورتر در امتدادِ آکسون پخش می‌شود و می‌تواند گرهٔ **بعدی** را تا آستانه دپلاریزه کند. پس پتانسیلِ عمل به‌جای بازتولیدِ پیوسته و کُند در هر نقطه، از گرهی به گرهِ دور **می‌جهد** (هدایتِ جهشی) و سرعتِ هدایت به‌شدت بالا می‌رود. کاهشِ ظرفیتِ خازنی هم شارژشدنِ غشا را سریع‌تر می‌کند و به همین سرعت کمک می‌کند.

!!! question "تمرینِ ۴"
    **(پروژه) بازتولیدِ همه‌یاهیچ با HH.** با مدلِ [هاجکین–هاکسلی](https://computational-neuroscience.ir/ch03/)، رفتارِ همه‌یاهیچ و آستانه را به‌صورتِ عددی بازتولید کنید: جریان‌های زیرآستانه و فراآستانه را تزریق کنید و نشان دهید که تنها دومی یک اسپایک می‌سازد.

??? success "راهنمای حل"
    از پیاده‌سازیِ هاجکین–هاکسلیِ [فصلِ HH](https://computational-neuroscience.ir/ch03/) استفاده کنید و یک پالسِ جریانِ کوتاه با دامنه‌های فزاینده تزریق کنید. انتظار: زیرِ یک **آستانهٔ** مشخص، \(V\) تنها یک برجستگیِ کوچکِ زیرآستانه می‌سازد و بازمی‌گردد؛ به‌محضِ عبور از آستانه، یک پتانسیلِ عملِ کامل با دامنهٔ تقریباً ثابت ظاهر می‌شود — و بزرگ‌ترکردنِ محرک دامنه را عوض نمی‌کند. اگر پالس را طولانی و ثابت کنید، به‌جای یک اسپایک یک **قطار** می‌گیرید که بسامدش با جریان زیاد می‌شود (منحنیِ f-I) — همان پیوندِ تمرینِ ۱ میانِ شدتِ محرک و بسامد.
