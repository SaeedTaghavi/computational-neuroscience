...
!!! question "تمرینِ ۱"
    **دمای اتاق.** ضریبِ \(\frac{RT}{F}\ln 10\) را برای دمای اتاق (\(T=293\,\mathrm{K}\)) دوباره حساب کنید. پتانسیل‌های تعادل چقدر تغییر می‌کنند؟
...
!!! question "تمرینِ ۲"
    **پتاسیمِ خارجی.** فرض کنید غلظتِ پتاسیمِ بیرونی از ۴ به ۸ میلی‌مولار برسد (مثلاً در آسیبِ بافتی). \(E_{\text{K}}\) و — با کدِ گلدمن — \(V_{\text{rest}}\) را دوباره حساب کنید. چرا افزایشِ پتاسیمِ خارجی، غشا را **دپلاریزه** می‌کند؟
...
!!! question "تمرینِ ۳"
    **حدِ گلدمن.** به‌صورتِ عددی نشان دهید که وقتی \(P_{\text{Na}}=P_{\text{Cl}}=0\)، خروجیِ `goldman` دقیقاً برابرِ `nernst` برای پتاسیم می‌شود.
...
!!! question "تمرینِ ۴"
    **منحنیِ گلدمن.** شکلِ \(V_m\) بر حسبِ \(P_{\text{Na}}/P_{\text{K}}\) را بازتولید کنید. نسبتی را بیابید که در آن \(V_m=0\) می‌شود. این نسبت چه معنای زیستی‌ای دارد؟
...
!!! question "تمرینِ ۵"
    **کلسیم و گلدمن.** با تابعِ `v_rest_numeric`، نشان دهید که با \(P_{\text{Ca}}=0\) خروجی دقیقاً با معادلهٔ بستهٔ `goldman` یکی می‌شود. سپس \(P_{\text{Ca}}\) را از صفر تا \(0.1\) جارو کنید و منحنیِ \(V_{\text{rest}}\) بر حسبِ \(P_{\text{Ca}}\) را رسم کنید (نمودارِ چپِ شکلِ کلسیم را بازتولید کنید). چرا این منحنی این‌قدر مسطح است؟
...
!!! question "تمرینِ ۶"
    **نیروی رانشِ کلسیم.** نیروی رانشِ \((V_{\text{rest}}-E_{\text{ion}})\) را برای هر چهار یون در استراحت حساب کنید و توضیح دهید چرا با وجودِ کوچک‌بودنِ سهمِ کلسیم در پتانسیلِ استراحت، ورودِ کلسیم می‌تواند یک **پیامِ** نیرومند باشد. (راهنما: به شیبِ غلظتیِ ده‌هزاربرابریِ کلسیم فکر کنید.)
...
!!! question "تمرینِ ۷"
    **(پروژه) نقشهٔ رژیم‌ها.** پتانسیلِ استراحت را بر حسبِ **دو** پارامترِ \(P_{\text{Na}}/P_{\text{K}}\) و \([\text{K}^+]_{\text{out}}\) به‌صورتِ یک نقشهٔ رنگی رسم کنید و نواحیِ «قطبیده» و «دپلاریزه» را مشخص کنید.
