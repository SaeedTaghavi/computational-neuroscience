...
روش به همان سادگی به سامانه‌های دوبعدی تعمیم می‌یابد. نوسانگرِ هماهنگِ میرا را در نظر بگیرید که یک جریانِ نوفه‌ای روی سرعتِ آن اثر می‌گذارد — نمونه‌ای ساده از یک سامانهٔ نوسانی که پیوسته تحتِ تأثیرِ نوفهٔ پس‌زمینه است:

$$
\begin{aligned}
dx &= v\,dt,\\
dv &= \big(-\omega^2 x - \gamma v\big)\,dt + \sigma\,dW.
\end{aligned}
$$

تنها معادلهٔ سرعت یک جملهٔ پخش دارد، چون نوفه روی نیرو (و نه مستقیماً روی مکان) وارد می‌شود. در شکلِ گسسته، فقط همان معادله یک گامِ $\sigma\sqrt{\Delta t}\,\xi$ می‌گیرد:

```python
def sho_step(x, v, dt, omega, gamma, sigma, rng):
    dW = rng.normal(0.0, np.sqrt(dt))      # Wiener increment ~ N(0, dt)
    x_new = x + v*dt
    v_new = v + (-omega**2 * x - gamma*v)*dt + sigma*dW   # noise on velocity
    return x_new, v_new
```

بدونِ نوفه، نوسانگر به‌سببِ میرایی به‌تدریج آرام می‌گیرد و در صفحهٔ فاز به‌سمتِ مبدأ مارپیچ می‌زند. با افزودنِ نوفه، سامانه هرگز کاملاً نمی‌ایستد: نوفه پیوسته آن را از تعادل بیرون می‌راند و یک نوسانِ پایدارِ نامنظم برقرار می‌ماند.

<figure markdown="span">
  ![Image title](em_sho.png)
  <figcaption>نوسانگر هماهنگ میرا با ورودی نوفه‌ای، حل‌شده با اویلر–مارویاما. چپ: نسخهٔ قطعی (خاکستری) میرا می‌شود اما نسخهٔ نوفه‌ای (آبی) پیوسته فعال می‌ماند. راست: در صفحهٔ فاز، مسیرِ قطعی به مبدأ مارپیچ می‌زند ولی مسیرِ نوفه‌ای ابری پیرامونِ آن می‌سازد.</figcaption>
</figure>
...
این مثال یک پدیدهٔ ژرف را آشکار می‌کند. جریانِ ورودیِ $I$ را زیرِ آستانهٔ شلیک انتخاب می‌کنیم؛ در نتیجه نسخهٔ قطعی روی نقطهٔ تعادلِ پایدار می‌ماند و کاملاً خاموش است. اما همین‌که نوفه را اضافه کنیم، تلنگرهای تصادفی هرازگاهی سامانه را از آستانه عبور می‌دهند و یک پتانسیل عملِ کامل برمی‌انگیزند. نوفه، نورونِ خاموش را به نورونی شلیک‌کننده بدل می‌کند — پدیده‌ای که به **شلیکِ القاشده با نوفه** مشهور است و نقشی بنیادی در رمزگذاریِ عصبی دارد.

<figure markdown="span">
  ![Image title](em_fhn.png)
  <figcaption>مدل فیتزهیو–ناگومو با جریان زیرآستانه. بالا: بدون نوفه، نورون روی تعادل می‌ماند و خاموش است. پایین: با همان ورودی به‌علاوهٔ نوفه، تلنگرهای تصادفی پتانسیل‌های عملِ کامل تولید می‌کنند — شلیکِ القاشده با نوفه.</figcaption>
</figure>

این دو مثال نشان می‌دهند که نوفه صرفاً «اخلال» نیست؛ گاهی رفتارِ کیفیِ تازه‌ای می‌آفریند که در سامانهٔ قطعی وجود ندارد.
