...
پیش از باز کردنِ پاسخ، خودتان دست به کار شوید: هر فیلتر را با `scipy.signal` بسازید و پاسخِ آن را وارسی کنید؛ سپس روی «پاسخ» کلیک کنید تا رویکرد و کدِ اجراشدنی را ببینید.

!!! question "تمرینِ ۱"
    **FIR در برابرِ IIR: طولِ پاسخِ ضربه‌ای.** یک فیلترِ FIR (میانگینِ متحرکِ ۱۱تایی) و یک فیلترِ IIR (باترورثِ مرتبهٔ چهار) را با یک ورودیِ ضربه $(1, 0, 0, \dots)$ تغذیه کنید. توضیح دهید چرا پاسخِ ضربه‌ای در یکی پس از شمارِ متناهیِ نمونه دقیقاً صفر می‌شود و در دیگری هرگز کاملاً صفر نمی‌شود، و این تفاوت از کجا می‌آید.

??? success "پاسخ"
    فیلترِ FIR تنها به مقادیرِ گذشتهٔ **ورودی** وابسته است؛ پس وقتی ضربه از پنجرهٔ متناهیِ آن (اینجا ۱۱ نمونه) بیرون رفت، خروجی دقیقاً صفر می‌شود. فیلترِ IIR جملهٔ **بازخورد** دارد (وابستگی به خروجی‌های گذشته)، پس هر ضربه در حلقهٔ بازخورد بازمی‌گردد و یک دنبالهٔ میرا—اما هرگز دقیقاً صفر—می‌سازد. همین ریشهٔ نام‌های «متناهی» و «نامتناهی» است.
    ...
    print("FIR nonzero samples:", np.count_nonzero(np.abs(h_fir) > 1e-12))
    print("FIR at n=11:", h_fir[11])
    print("IIR at n=40:", h_iir[40])
    ```

    خروجی نشان می‌دهد پاسخِ FIR دقیقاً ۱۱ نمونهٔ ناصفر دارد و از $n=11$ به بعد صفر است، حال آنکه پاسخِ IIR در $n=40$ هنوز ناصفر است (حدودِ $-6.4\times 10^{-4}$) و تنها به‌تدریج میرا می‌شود.

!!! question "تمرینِ ۲"
    **طراحیِ فیلترِ میان‌گذر برای باندِ آلفا.** یک فیلترِ میان‌گذرِ باترورثِ مرتبهٔ چهار برای جداکردنِ باندِ آلفای مغزی (۸ تا ۱۲ هرتز) با بسامدِ نمونه‌برداریِ $f_s = 500$ هرتز طراحی کنید. پاسخِ بسامدیِ آن را با `freqz` به‌دست آورید و بهرهٔ فیلتر را در ۱۰ هرتز (میانهٔ باند) و در ۴ و ۲۰ هرتز (بیرونِ باند) گزارش کنید.

??? success "پاسخ"
    ضرایب را با `butter` و آرگومانِ `btype='band'` و فهرستِ دو مرزی می‌سازیم، سپس پاسخِ بسامدی را نمونه‌برداری می‌کنیم و بهره را در بسامدهای دلخواه می‌خوانیم:
    ...
    fs = 500.0
    b, a = sig.butter(4, [8, 12], btype="band", fs=fs)
    w, h = sig.freqz(b, a, fs=fs, worN=8192)

    def gain_at(f):
        return np.abs(h[np.argmin(np.abs(w - f))])

    for f in (4, 8, 10, 12, 20):
        print(f, round(float(gain_at(f)), 4))
    ```

    بهره در ۱۰ هرتز تقریباً $1.0$ (عبورِ کامل)، و در ۴ و ۲۰ هرتز به‌ترتیب حدودِ $0.002$ و $0.005$ است (تضعیفِ شدید). نکتهٔ آموزنده: در خودِ مرزها (۸ و ۱۲ هرتز) بهره حدودِ $0.707$ است—یعنی همان نقطهٔ $-3$ دسی‌بل که بسامدِ مرزیِ باترورث بنا به تعریف در آنجاست. پس «باندِ عبور» تا جایی می‌رسد که توان به نیمه افت کند، نه یک لبهٔ تیز.

!!! question "تمرینِ ۳"
    **حذفِ نوفهٔ خطِ برق با فیلترِ میان‌نگذر.** سیگنالی بسازید که از دو ریتمِ مغزیِ ۱۰ و ۲۲ هرتزی به‌علاوهٔ یک نوفهٔ ۵۰ هرتزیِ خطِ برق تشکیل شده باشد. با `scipy.signal.iirnotch` یک فیلترِ میان‌نگذر در ۵۰ هرتز طراحی و اعمال کنید، و با روشِ ولچ نشان دهید که قلهٔ ۵۰ هرتزی چقدر پایین می‌آید در حالی‌که ریتم‌های مغزی دست‌نخورده می‌مانند.

??? success "پاسخ"
    فیلترِ میان‌نگذر تنها یک باندِ باریک (با پهنای تقریبیِ $f_0/Q$) را حذف می‌کند و بقیهٔ طیف را رها می‌گذارد. آن را با `filtfilt` بدونِ جابه‌جاییِ فاز اعمال می‌کنیم و توان را پیش و پس با ولچ می‌سنجیم:
    ...
    fs = 1000.0
    t = np.arange(0, 3, 1/fs)
    brain = np.sin(2*np.pi*10*t) + 0.6*np.sin(2*np.pi*22*t)
    contaminated = brain + 1.2*np.sin(2*np.pi*50*t)     # add 50 Hz mains

    b, a = sig.iirnotch(50, Q=30, fs=fs)                 # bandwidth ~ 50/30 Hz
    cleaned = sig.filtfilt(b, a, contaminated)

    f, P_c = sig.welch(contaminated, fs=fs, nperseg=1024)
    _, P_k = sig.welch(cleaned,      fs=fs, nperseg=1024)
    i50 = np.argmin(np.abs(f - 50))
    print("PSD @ 50 Hz  before:", P_c[i50], " after:", P_k[i50])
    print("reduction factor:", round(float(P_c[i50]/P_k[i50])))
    ```

    توانِ ۵۰ هرتزی بیش از سه مرتبهٔ بزرگی (حدودِ ۱۵۰۰ برابر) کاهش می‌یابد، در حالی‌که قله‌های ۱۰ و ۲۲ هرتزی تقریباً بی‌تغییر می‌مانند—چون پهنای حذفِ فیلتر تنها حدودِ $f_0/Q \approx 1.7$ هرتز است و به آن بسامدها نمی‌رسد.

!!! question "تمرینِ ۴"
    **پروژه: انتخابِ مرزِ فیلترِ بالاگذر برای یافتنِ اسپایک.** در بخشِ «فیلترِ بالاگذر برای یافتنِ اسپایک‌ها» از مرزِ ۳۰۰ هرتز استفاده کردیم. با کدِ همان بخش آزمایش کنید: مرز را به ۱۰۰، ۳۰۰ و ۶۰۰ هرتز تغییر دهید و ببینید شمارِ اسپایک‌های یافته‌شده و شکلِ خطِ پایه چگونه عوض می‌شود. توضیح دهید چرا مرزِ خیلی پایین (نزدیک به باندِ LFP) و مرزِ خیلی بالا هر دو زیان‌بارند.

??? success "راهنمای حل"
    این یک مسئلهٔ بازِ «طراحیِ تصمیم» است؛ پاسخِ یکتا ندارد، اما استدلال روشن است. تابعِ `butter_filter` و کدِ آشکارسازیِ اسپایکِ همان بخش را بردارید و تنها آرگومانِ مرز را در یک حلقه تغییر دهید:

    ```python
    for fc in (100, 300, 600):
        hp = butter_filter(raw, fs, fc, "high")
        sigma = np.median(np.abs(hp)) / 0.6745
        # ... same threshold-and-refractory detection ...
        print(fc, "Hz -> detected", len(detected), "spikes")
    ```

    انتظار داریم: با مرزِ **خیلی پایین** (مثلاً ۱۰۰ هرتز) بخشی از نوسانِ آهستهٔ LFP از فیلتر می‌گذرد، خطِ پایه هنوز موج می‌خورد و آستانهٔ ثابت اسپایک‌های کاذب یا جاافتاده می‌دهد. با مرزِ **خیلی بالا** (مثلاً ۶۰۰ هرتز) بخشی از انرژیِ خودِ شکل‌موجِ اسپایک (که پهنای بسامدیِ محدودی دارد) نیز حذف می‌شود، دامنهٔ اسپایک افت می‌کند و برخی اسپایک‌ها زیرِ آستانه می‌مانند. مرزِ مرسومِ حدودِ ۳۰۰ هرتز یک بده‌بستان است: به‌قدری بالا که LFP را کامل بزداید و به‌قدری پایین که تنِ اصلیِ اسپایک را نگه دارد. همین «طراحیِ آگاهانهٔ مرز بر پایهٔ محتوای بسامدیِ سیگنالِ هدف»، جانِ کلامِ کاربردِ فیلتر است.
