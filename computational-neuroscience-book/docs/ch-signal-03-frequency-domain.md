...
پیش از باز کردنِ پاسخ، خودتان دست به کار شوید: مسئله را روی کاغذ یا با چند خط کد حل کنید، و آنگاه روی «پاسخ» کلیک کنید تا راهِ‌حل و—آنجا که کد کمک می‌کند—قطعهٔ اجراشدنیِ آن را ببینید.

!!! question "تمرینِ ۱"
    **سری فوریهٔ موجِ مربعی.** موجِ مربعیِ متناوب با دورهٔ $T_0$ را در نظر بگیرید:
...
??? success "پاسخ"
    ...
    **سری کامل.** پس موجِ مربعی تنها از هم‌نواهای فرد ساخته می‌شود:
    ...
    برای اطمینان، همین ضرایب را عددی نیز می‌سنجیم؛ انتگرال‌ها را با یک جمعِ ریمانِ ریز تقریب می‌زنیم:
    ...
    T0 = 1.0
    w0 = 2*np.pi / T0
    t = np.linspace(0, T0, 200000, endpoint=False)
    dt = t[1] - t[0]
    x = np.where(t < T0/2, 1.0, -1.0)          # one period of the square wave

    for k in range(1, 8):
        bk = 2/T0 * np.sum(x*np.sin(k*w0*t)) * dt
        print(k, round(bk, 4), round(4/(k*np.pi), 4))
    ```

    خروجی می‌دهد $b_1 \approx 1.273$، $b_3 \approx 0.424$، $b_5 \approx 0.255$ (یعنی همان $4/(k\pi)$) و برای $k$ زوج تقریباً صفر—دقیقاً مطابقِ فرمولِ تحلیلی. (بالاتر در همین بخش، ساختِ عددیِ همین سری و هم‌گراییِ آن به موجِ مربعی—همراه با پدیدهٔ گیبس—را با کد دیدیم.)

!!! question "تمرینِ ۲"
    **تقارن و حذفِ ضرایب.** بدونِ محاسبهٔ هیچ انتگرالی، توضیح دهید که برای هر یک از حالت‌های زیر کدام دسته از ضرایب لزوماً صفرند:
...
!!! question "تمرینِ ۳"
    **فاز و تأخیرِ زمانی.** سیگنالِ $x(t) = A\cos(2\pi f_0 t + \theta)$ را با $f_0 = 10$ هرتز و $\theta = \pi/2$ در نظر بگیرید. تأخیرِ زمانیِ $\theta_t$ ناشی از این فاز را بیابید. اگر بسامد به $f_0 = 40$ هرتز افزایش یابد (و فاز ثابت بماند)، تأخیرِ زمانی چه می‌شود؟
