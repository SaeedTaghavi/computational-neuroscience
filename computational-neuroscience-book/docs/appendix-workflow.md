...
طراحیِ یک آزمایشِ محاسباتیِ خوب، پیش از نوشتنِ کد آغاز می‌شود. پرسش‌های زیر همان تصمیم‌هایی‌اند که خوب است پیش از شروع به آن‌ها فکر کنیم. برای هر پرسش نخست خودتان کمی بیندیشید و پاسخِ خود را بنویسید، سپس روی «پاسخ» کلیک کنید و آن را با آنچه در ادامه آمده بسنجید.

!!! question "پرسشِ ۱"
    **انتخابِ پارامترها.** کدام پارامترها را جاروب کنیم و در چه بازه‌ای؟

??? success "پاسخ"
    هر پارامترِ افزوده، شمارِ اجراها را چندبرابر می‌کند (نفرینِ ابعاد). جاروبِ سه پارامتر با ده مقدار هرکدام، هزار اجرا می‌شود. باید آگاهانه انتخاب کنیم که کدام پارامترها مهم‌اند و چه گامی میانِ مقادیر معقول است (خطی یا لگاریتمی).

!!! question "پرسشِ ۲"
    **انتخابِ آنچه ذخیره می‌شود.** چه چیزی را ذخیره کنیم؟

??? success "پاسخ"
    آیا به کلِ مسیر نیاز داریم یا تنها چند کمیتِ خلاصه؟ ذخیرهٔ کلِ مسیرها برای هزاران اجرا می‌تواند گیگابایت‌ها فضا بگیرد. گاه بهتر است تنها کمیت‌های خلاصه را نگه داریم، و کلِ مسیر را فقط برای چند اجرای منتخب ذخیره کنیم.

!!! question "پرسشِ ۳"
    **تضمینِ بازتولیدپذیری.** چگونه بازتولیدپذیری را تضمین کنیم؟

??? success "پاسخ"
    مقدارِ اولیهٔ تصادفی را صریح و ثبت‌شده نگه می‌داریم. همهٔ پارامترها را در فراداده می‌نویسیم. نسخهٔ کد و کتابخانه‌ها را ثبت می‌کنیم. هدف این است که هر کسی (از جمله خودِ ما در آینده) بتواند دقیقاً همان نتیجه را بازتولید کند.

!!! question "پرسشِ ۴"
    **استقلالِ اجراها.** آیا شبیه‌سازی‌ها مستقل‌اند؟

??? success "پاسخ"
    اگر اجراها به هم وابسته نباشند (که در جاروبِ پارامترها معمولاً چنین است)، می‌توان آن‌ها را **موازی** اجرا کرد و زمانِ محاسبه را به‌شدت کاهش داد. طراحیِ تابعِ مستقلِ یک‌شبیه‌سازی، این موازی‌سازی را بسیار آسان می‌کند.

!!! question "پرسشِ ۵"
    **بررسیِ همگرایی.** همگراییِ عددی را چگونه بررسی کنیم؟

??? success "پاسخ"
    پیش از اعتماد به نتایجِ یک جاروبِ بزرگ، باید مطمئن شویم که گامِ زمانیِ ($\Delta t$) به‌قدرِ کافی کوچک است. خوب است آزمایشی کوچک انجام دهیم: یک اجرا را با چند گامِ زمانیِ کاهنده تکرار کنیم و ببینیم که آیا نتیجه به مقداری پایدار همگرا می‌شود یا نه (همان اصلِ اعتبارسنجیِ فصلِ معادلاتِ دیفرانسیل). برای نمونه، دامنهٔ پایدارِ همان نوسانگرِ محرک‌دار را برای چند گامِ زمانیِ کاهنده حساب می‌کنیم:

    ```python
    import numpy as np

    def steady_amplitude(dt, gamma=0.1, F=0.5, omega0=1.0, omega_d=1.0, T=120.0):
        n = int(T/dt); t = np.arange(n)*dt
        x = np.zeros(n); v = np.zeros(n)
        for i in range(n-1):
            a = -2*gamma*v[i] - omega0**2*x[i] + F*np.cos(omega_d*t[i])
            v[i+1] = v[i] + a*dt
            x[i+1] = x[i] + v[i+1]*dt
        return np.std(x[n//2:])

    for dt in [0.1, 0.05, 0.02, 0.01, 0.005]:
        print(f"dt={dt:<6} amplitude={steady_amplitude(dt):.4f}")
    ```

    خروجی نشان می‌دهد که پاسخ به‌سرعت پایدار می‌شود و از یک جایی به بعد کوچک‌ترکردنِ گامِ زمانی تغییرِ معناداری نمی‌دهد:

    ```text
    dt=0.1    amplitude=1.7632
    dt=0.05   amplitude=1.7630
    dt=0.02   amplitude=1.7629
    dt=0.01   amplitude=1.7629
    dt=0.005  amplitude=1.7628
    ```

    تفاوتِ میانِ گامِ ۰٫۱ و گامِ ۰٫۰۰۵ کمتر از یک‌دهمِ درصد است؛ پس گامِ ۰٫۰۱ برای این جاروب به‌قدرِ کافی کوچک است و کوچک‌ترکردنِ بیشترِ آن تنها هزینهٔ محاسبه را بالا می‌برد بی‌آنکه دقت را معنادار کند.

!!! question "پرسشِ ۶"
    **ساختارِ پوشه‌ها و نام‌گذاری.** ساختارِ پوشه‌ها و نام‌گذاری چگونه باشد؟

??? success "پاسخ"
    یک ساختارِ پوشهٔ روشن (مثلاً پوشه‌ای جداگانه برای هر آزمایش، با داده، فراداده و کدِ مولد در کنارِ هم) بازگشت به کار را در آینده بسیار آسان می‌کند. یک چیدمانِ خوب، کد، داده و فراداده را از هم جدا اما به هم پیوسته نگه می‌دارد. برای مثال:
