...
نخست بکوشید هر مسئله را با توابعِ همین فصل خودتان حل کنید؛ سپس پاسخ یا راهنمای حل را بگشایید.

!!! question "تمرینِ ۱"
    **(تمرین) شمارشِ تعادل‌ها و آستانهٔ دوپایداری.** نمودارِ فاز را برای \(\beta=2\) و \(\beta=10\) بازتولید کنید و با شمارشِ تقاطعِ نولکلین‌ها تأیید کنید که گذار به دوپایداری رخ داده. آستانهٔ تقریبیِ \(\beta\) را با ادامهٔ عددی بیابید.

??? success "پاسخ"
    برای شمارشِ تعادل‌ها به‌جای رسمِ چشمی، از همان `find_unique_equilibria` فصل استفاده می‌کنیم و از شبکه‌ای از حدس‌های آغازین شروع می‌کنیم. برای یافتنِ آستانه، تعادلِ متقارنِ \(u=v=s\) با \(s(1+s^\beta)=1\) را در نظر می‌گیریم: این نقطه هنگامی به زین بدل می‌شود (یعنی دوپایداری آغاز می‌شود) که جفتیدگیِ خارج‌قطریِ ژاکوبین، \(g=\beta\,s^{\beta-1}/(1+s^\beta)^2\)، از ۱ بگذرد. ریشه‌یابیِ \(g(\beta)=1\) آستانه را می‌دهد.

    ```python
    import numpy as np
    from scipy.optimize import brentq

    def s_star(beta):                       # symmetric equilibrium, alpha = 1
        return brentq(lambda s: s * (1 + s**beta) - 1, 1e-6, 1.0)

    def coupling(beta):                     # off-diagonal Jacobian term at the symmetric point
        s = s_star(beta)
        return beta * s**(beta - 1) / (1 + s**beta)**2

    beta_star = brentq(lambda b: coupling(b) - 1, 1.5, 8.0)
    print("bistability threshold beta* =", beta_star)
    for b in (2, 10):
        print(f"beta={b}: coupling={coupling(b):.2f}",
              "-> bistable" if coupling(b) > 1 else "-> monostable")
    ```

    خروجی: آستانه \(\beta^\ast\approx 4.14\). برای \(\beta=2\) جفتیدگی \(0.64<1\) است (یک تعادل، تک‌پایدار) و برای \(\beta=10\) جفتیدگی \(1.56>1\) (سه تعادل، دوپایدار) — دقیقاً همان چیزی که شمارشِ تقاطعِ نولکلین‌ها نشان می‌دهد. این آستانه همان نقطهٔ انشعابِ چنگالیِ فوق‌بحرانیِ نمودارِ بخشِ ۸ است (\(\beta\approx 4\)).

!!! question "تمرینِ ۲"
    **(تمرین) از دو شرطِ اولیهٔ نزدیک به هم اما در دو سوی زین شبیه‌سازی کنید و نشان دهید به حالت‌های پایدارِ متفاوت می‌رسند. این همان *به‌خاطرسپردنِ* ورودی است.**

??? success "پاسخ"
    برای \(\beta=10\) تعادلِ زین همان نقطهٔ متقارنِ \(u=v\approx 0.844\) است (از `s_star(10)`). دو شرطِ اولیهٔ بسیار نزدیک را درست در دو سوی این زین می‌گیریم و با `odeint` و تابعِ `cellular_switch` فصل پیش می‌رانیم.
    ...
    saddle = 0.844                                   # symmetric saddle for beta=10
    for eps in (-0.02, +0.02):
        ic = (saddle + eps, saddle - eps)            # just across the saddle
        end = scipy.integrate.odeint(
            partial(cellular_switch, alpha=1, beta=10), y0=ic, t=time)[-1]
        print(f"IC {ic} -> {np.round(end, 3)}")
    ```

    خروجی: یکی به حالتِ «\(u\) بالا، \(v\) پایین» (\(\approx(1.0,\,0.5)\)) و دیگری به حالتِ آینه‌ایِ آن (\(\approx(0.5,\,1.0)\)) می‌رسد. جابه‌جاییِ ناچیزِ آغازین سرنوشتِ نهایی را تعیین می‌کند: زین مرزِ میانِ دو حوضهٔ جذب است، و همین حساسیت به شرطِ آغازین، معنای *ذخیرهٔ یک بیتِ حافظه* است.

!!! question "تمرینِ ۳"
    **(پروژه)** مدلِ نامتقارن را پیاده کنید و نقشهٔ کاملِ \((\beta_1,\beta_2)\) را بسازید. نوکِ کاسپ را مکان‌یابی کنید. سپس یک محرکِ گذرا به یکی از متغیرها بدهید و نشان دهید چگونه می‌توان دستگاه را میانِ دو حالت **سوئیچ** کرد (همان عملیاتِ نوشتنِ حافظه).

??? success "راهنمای حل"
    این پروژه سه گام دارد؛ همه با ابزارهای همین فصل ساختنی‌اند.

    - **نقشهٔ \((\beta_1,\beta_2)\):** جریانِ نامتقارنِ بخشِ ۹ را پیاده کنید و برای هر جفتِ \((\beta_1,\beta_2)\) روی یک شبکه، شمارِ تعادل‌های پایدار را بشمارید (همان `find_unique_equilibria` + `stability`). خانه‌ها را بر پایهٔ «یک حالت» یا «دو حالت» رنگ کنید تا ناحیهٔ دوپایدار پدیدار شود.
    - **نوکِ کاسپ:** مرزِ ناحیهٔ دوپایدار از دو خمِ زین–گره ساخته شده که در نوکِ کاسپ به‌هم می‌رسند، نزدیکِ \(\beta_1=\beta_2\approx 4\) (همان آستانهٔ متقارنِ مسئلهٔ ۱). با ریزکردنِ شبکه در آن ناحیه، نوک را دقیق‌تر مکان‌یابی کنید.
    - **نوشتنِ حافظه:** پارامترها را در ناحیهٔ دوپایدار ثابت کنید، دستگاه را در یک حالت بنشانید، سپس در یک بازهٔ کوتاه یک جملهٔ محرکِ بزرگ به معادلهٔ \(u\) بیفزایید (مثلاً `I_pulse` برای \(t\in[t_1,t_2]\)) تا مسیر از مرزِ زین بگذرد و در حالتِ دیگر بنشیند. ردِ \(u(t)\) باید پرشِ پلکانیِ ماندگار میانِ دو حالت را نشان دهد.

!!! question "تمرینِ ۴"
    **(پروژه)** نوفه را به مدل بیفزایید (با اویلر–مارویاما، فصلِ [SDE](https://computational-neuroscience.ir/ch-num-07-sde/)) و بررسی کنید که نوفه چگونه می‌تواند دستگاه را به‌طورِ خودبه‌خودی میانِ دو حالت بپراند (سوئیچِ القاشده با نوفه) — پدیده‌ای که در بیانِ ژن و در ادراکِ دوحالته اهمیت دارد.

??? success "راهنمای حل"
    جریانِ قطعی را با یک جملهٔ نوفه بیامیزید و با اویلر–مارویاما حل کنید (همان طرحی که در فصلِ فیتزهیو–ناگومو نیز به کار می‌رود): در هر گام \(\;y \leftarrow y + F(y)\,dt + \sigma\,\sqrt{dt}\,\xi\;\) با \(\xi\sim\mathcal{N}(0,1)\). دستگاه را در ناحیهٔ دوپایدار (\(\beta=10\)) نگه دارید و دامنهٔ نوفهٔ \(\sigma\) را جارو کنید:

    - برای \(\sigma\) کوچک، دستگاه در یک حوضهٔ جذب می‌لرزد و به‌ندرت می‌پرد.
    - با افزایشِ \(\sigma\)، گاه‌به‌گاه یک تلنگرِ نوفه‌ای، مسیر را از فرازِ زین می‌گذراند و به حوضهٔ دیگر می‌اندازد — **پرش‌های خودبه‌خودی** میانِ دو حالت.
    - آهنگِ این پرش‌ها با \(\sigma\) به‌شدت (به‌صورتِ نمایی، طبقِ قانونِ کرامرس) افزایش می‌یابد.

    یک نمودارِ سریِ‌زمانیِ \(u(t)\) که پرش‌های نامنظمِ دوحالته را نشان دهد، شاهدِ خوبی برای این پدیده است. همین سازوکار، پایهٔ *تغییرپذیریِ سلول‌به‌سلولِ* بیانِ ژن و *وارونگیِ ادراکِ* دوحالته است.
