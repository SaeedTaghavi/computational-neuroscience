...
نخست خودتان دست‌به‌قلم و دست‌به‌کد ببرید؛ سپس پاسخِ هر تمرین را برای مقایسه بگشایید.

!!! question "تمرینِ ۱"
    **حل‌کن‌و‌بسنج.** برای \(\dot x = x^2 - 1\) با \(x(0)=1\)، جوابِ دقیق چیست؟ (راهنمایی: \(x_0\) یک نقطهٔ ثابت است.) با اویلرِ پیشرو، \(dt=0.01\) و \(T=5\)، عددی تأیید کنید.

??? success "پاسخ"
    چون \(f(1)=1^2-1=0\)، خودِ \(x=1\) یک نقطهٔ ثابت است. طبق اصلِ بنیادیِ همین فصل، جوابی که از یک نقطهٔ ثابت آغاز شود برای همیشه همان‌جا می‌ماند؛ پس جوابِ دقیق تابعِ ثابتِ \(x(t)=1\) است. اویلرِ پیشرو نیز باید عیناً روی همان مقدار بماند، چون در هر گام \(\dot x = f(1)=0\) است.
    ...
    f = lambda x: x**2 - 1
    print(euler(f, 1.0, 0.01, 5.0))   # -> 1.0  (روی نقطهٔ ثابت می‌ماند)
    print(euler(f, 0.9, 0.01, 5.0))   # -> -1.0 (به نقطهٔ پایدار می‌لغزد)
    ```

    نتیجهٔ عددی دقیقاً \(1.0\) است. کوچک‌ترین جابه‌جایی به زیرِ \(1\) (مثلاً \(x_0=0.9\)) به‌سوی نقطهٔ پایدارِ \(-1\) رانده می‌شود و هر آغازِ بالای \(1\) به بی‌نهایت می‌گریزد؛ همان ناپایداریِ \(x^*=+1\) که خطِ فاز پیش‌بینی می‌کند.

!!! question "تمرینِ ۲"
    **خطِ فاز.** خطِ فازِ \(\dot x = -(x+1)(x-1)(x-2)\) را رسم کنید. هر سه نقطهٔ ثابت را بیابید و هرکدام را با علامتِ \(f'(x^*)\) رده‌بندی کنید. با شبیه‌سازی از چند شرطِ اولیه راستی‌آزمایی کنید.

??? success "پاسخ"
    نقاطِ ثابت جایی‌اند که \(f\) صفر می‌شود: \(x^*=-1,\ 1,\ 2\). با محاسبهٔ علامتِ \(f'(x^*)\) به‌دست می‌آید \(f'(-1)=-6<0\) (پایدار)، \(f'(1)=+2>0\) (ناپایدار) و \(f'(2)=-3<0\) (پایدار). پس دو نقطهٔ پایدار در \(-1\) و \(2\) با یک نقطهٔ ناپایدارِ میانی در \(1\) که مرزِ دو حوضهٔ جذب را می‌سازد؛ این یک نمونهٔ **دوپایداری** است. برای رسمِ خطِ فاز همان تابعِ `phase_line` این فصل را بازبه‌کار می‌بریم.

    ```python
    f = lambda x: -(x + 1) * (x - 1) * (x - 2)
    phase_line(f, xrange=(-1.6, 2.6))   # تابعِ همین فصل

    # راستی‌آزمایی با شبیه‌سازی از چند شرطِ اولیه
    for x0 in (-1.5, 0.0, 0.5, 1.2, 1.5, 2.5):
        print(x0, "->", euler(f, x0, 0.001, 30))
    ```

    شبیه‌سازی تأیید می‌کند: هر آغاز زیرِ \(1\) به \(-1\) و هر آغاز بالای \(1\) به \(2\) می‌رسد. علامتِ \(f'\) و تصویرِ خطِ فاز هر دو یک داستان را می‌گویند.

!!! question "تمرینِ ۳"
    **توانِ سوم.** نشان دهید \(x^*=0\) برای \(\dot x = x^3\) ناپایدار است، هرچند \(f'(0)=0\). چرا آزمونِ خطی اینجا شکست می‌خورد و چرا خطِ فاز همچنان کار می‌کند؟

??? success "پاسخ"
    آزمونِ خطی به علامتِ \(f'(x^*)\) تکیه دارد، اما اینجا \(f'(x)=3x^2\) و \(f'(0)=0\)؛ جملهٔ خطی محو می‌شود و هیچ نمی‌گوید. پس باید مستقیماً به خطِ فاز نگاه کنیم: برای \(x>0\) داریم \(f(x)=x^3>0\) (جریان به راست، دورشونده از صفر) و برای \(x<0\) داریم \(f(x)<0\) (جریان به چپ، باز دورشونده). هر دو پیکان از \(0\) به بیرون اشاره می‌کنند، پس \(x^*=0\) **ناپایدار** است.

    ```python
    f = lambda x: x**3
    for x0 in (0.1, -0.1):
        print(x0, "->", euler(f, x0, 1e-4, 10))   # 0.1 -> 0.112 ,  -0.1 -> -0.112
    ```

    مقادیر از صفر دور می‌شوند و ناپایداری را تأیید می‌کنند. درسِ ماجرا همان جملهٔ فصل است: هرگاه \(f'(x^*)=0\) شد، رسمِ خطِ فاز از به‌خاطرسپردنِ قاعدهٔ خطی مطمئن‌تر است.

!!! question "تمرینِ ۴"
    **انشعاب را بیاب.** برای \(\dot x = x^2 + a\)، نقاطِ ثابت را برای \(a=-1\)، \(a=0\) و \(a=1\) رده‌بندی کنید و تأیید کنید که انشعاب در \(a=0\) است.

??? success "پاسخ"
    نقاطِ ثابت از \(x^2+a=0\) یعنی \(x^*=\pm\sqrt{-a}\) به‌دست می‌آیند و علامتِ \(f'(x)=2x\) پایداری را تعیین می‌کند. برای \(a=-1\) دو نقطه هست: \(x^*=-1\) با \(f'=-2<0\) (پایدار) و \(x^*=+1\) با \(f'=+2>0\) (ناپایدار). برای \(a=0\) این دو در \(x^*=0\) با \(f'=0\) ادغام می‌شوند (نیمه‌پایدار). برای \(a=1\) هیچ نقطهٔ ثابتِ حقیقی نمی‌ماند. پس تعدادِ نقاطِ ثابت در \(a=0\) از دو به صفر می‌پرد: این همان انشعابِ **زین–گره** است.
    ...
    for a in (-1.0, 0.0, 1.0):
        disc = -a
        if disc > 0:
            for xs in (-np.sqrt(disc), np.sqrt(disc)):
                print(f"a={a:+.0f}  x*={xs:+.2f}  f'={2*xs:+.2f}")
        elif disc == 0:
            print(f"a={a:+.0f}  x*=0.00  f'=0.00  (نیمه‌پایدار)")
        else:
            print(f"a={a:+.0f}  بدونِ نقطهٔ ثابتِ حقیقی")
    ```

    خروجی نشان می‌دهد دو نقطهٔ ثابت (یکی پایدار، یکی ناپایدار) در \(a=0\) با هم برخورد و ناپدید می‌شوند؛ مقدارِ بحرانیِ انشعاب دقیقاً \(a=0\) است.
