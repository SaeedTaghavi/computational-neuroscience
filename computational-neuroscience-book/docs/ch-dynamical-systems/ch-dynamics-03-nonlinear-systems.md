...
نخست خودتان نقاطِ ثابت و ژاکوبین را بیرون بکشید، سپس پاسخ را برای مقایسه بگشایید.

!!! question "تمرینِ ۱"
    برای دستگاهِ غیرخطیِ \(\dot x = -(1-y^2)x - y,\; \dot y = x\)، نقطهٔ ثابتِ یکتا را بیابید، ژاکوبین را آنجا محاسبه و آن را رده‌بندی کنید (گره، زین یا مارپیچ). با شبیه‌سازی تأیید کنید.

??? success "پاسخ"
    از \(\dot y=x=0\) نتیجه می‌شود \(x=0\)، و آن‌گاه \(\dot x=-y=0\) یعنی \(y=0\)؛ پس تنها نقطهٔ ثابت مبدأ \((0,0)\) است. ژاکوبین در حالتِ کلی \(J=\begin{bmatrix} -(1-y^2) & 2xy-1 \\ 1 & 0 \end{bmatrix}\) است که در مبدأ به \(J=\begin{bmatrix} -1 & -1 \\ 1 & 0 \end{bmatrix}\) می‌رسد. با \(\operatorname{Tr}(J)=-1\)، \(\det(J)=1\) و \(T^2-4D=-3<0\)، این یک **مارپیچِ پایدار** است. همان تابعِ `classify` این فصل را بازبه‌کار می‌بریم.
    ...
    def flow(u, t=0):
        x, y = u
        return np.array([-(1 - y**2) * x - y, x])

    J = np.array([[-1., -1.], [1., 0.]])
    print(classify(J))                       # ('stable spiral', True)

    t = np.linspace(0, 40, 4000)
    end = scipy.integrate.odeint(flow, [0.3, 0.3], t)[-1]
    print(end)                                # ~ [0, 0]
    ```

    شبیه‌سازی از \((0.3,0.3)\) به‌صورتِ مارپیچی به مبدأ میرا می‌شود و رده‌بندیِ «مارپیچِ پایدار» را تأیید می‌کند.

!!! question "تمرینِ ۲"
    **** مدلِ رقابت را با ضرایبِ متفاوت بازنویسی کنید تا نقطهٔ هم‌زیستیِ \((1,1)\) به یک ** گرهِ پایدار** بدل شود (هم‌زیستیِ پایدار). کدامین تغییر در ضرایبِ نولکلین‌ها این را ممکن می‌کند؟

??? success "پاسخ"
    در مدلِ اصلی، رقابتِ **بین‌گونه‌ای** (ضریبِ متقابلِ \(2\)) از رقابتِ **درون‌گونه‌ای** (ضریبِ \(1\) روی خودِ گونه) قوی‌تر بود و همین \((1,1)\) را به زین بدل می‌کرد. اگر رابطه را وارونه کنیم — رقابتِ درون‌گونه‌ای قوی‌تر از بین‌گونه‌ای — هم‌زیستی پایدار می‌شود. مثلاً \(\dot x = x\,(3-2x-z),\ \dot z = z\,(3-x-2z)\): نولکلین‌های درونی \(2x+z=3\) و \(x+2z=3\) باز در \((1,1)\) قطع می‌شوند، اما اکنون ژاکوبین در آنجا \(\begin{bmatrix} -2 & -1 \\ -1 & -2 \end{bmatrix}\) است با \(\operatorname{Tr}=-4\)، \(\det=3\) و \(T^2-4D=4>0\): یک **گرهِ پایدار**.

    ```python
    def comp2(u, t=0):
        x, z = u
        return np.array([x * (3 - 2*x - z), z * (3 - x - 2*z)])

    def jac2(x, z):
        return np.array([[3 - 4*x - z, -x],
                         [-z,          3 - x - 4*z]])

    print(classify(jac2(1, 1)))              # ('stable node', True)
    t = np.linspace(0, 40, 4000)
    for u0 in ([0.5, 0.6], [1.5, 0.2], [0.2, 1.5]):
        print(u0, "->", scipy.integrate.odeint(comp2, u0, t)[-1])   # همه به (1,1)
    ```

    اکنون از هر شرطِ اولیه‌ای مسیرها به \((1,1)\) همگرا می‌شوند: کاهشِ ضرایبِ متقابلِ نولکلین‌ها (از \(2\) به \(1\)) هم‌زیستی را از ناپایدار به پایدار می‌برد.

!!! question "تمرینِ ۳"
    تابعِ `classify` را گسترش دهید تا حالت‌های مرزی ( \(\det J \approx 0\) یا \(\operatorname{Tr} J \approx 0\) ) را نیز برچسب بزند، چون این‌ها دقیقاً جایی‌اند که انشعاب‌ها رخ می‌دهند.

??? success "پاسخ"
    کافی است پیش از رده‌بندیِ عادی، دو مرزِ انشعاب را با یک آستانهٔ کوچک وارسی کنیم: \(\det J\approx 0\) لبهٔ **زین–گره** است (یک مقدارِ ویژه به صفر می‌رسد) و \(\operatorname{Tr} J\approx 0\) با \(\det J>0\) لبهٔ **هاپف** (زوجی از مقادیرِ ویژه از محورِ موهومی می‌گذرند).

    ```python
    def classify(J, tol=1e-6):
        T, D = np.trace(J), np.linalg.det(J)
        if abs(D) < tol:
            return "saddle-node (D~0)", None
        if D > 0 and abs(T) < tol:
            return "Hopf (T~0)", None
        if D < 0:
            return "saddle", False
        stable = T < 0
        shape = "spiral" if T**2 - 4*D < 0 else "node"
    ...
    print(classify(np.array([[0., 1.], [-1., 0.]])))   # Hopf (T~0)
    print(classify(np.array([[0., 1.], [0., -1.]])))   # saddle-node (D~0)
    ```

    این نسخهٔ گسترش‌یافته دقیقاً همان مرزهایی را که در بخشِ «انشعاب در دستگاه‌ها» توصیف شد نشانه‌گذاری می‌کند و برای پیمایشِ پارامتری و یافتنِ نقاطِ انشعاب سودمند است.
