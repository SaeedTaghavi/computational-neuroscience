...
!!! note "هاپف تنها یکی از دو راهِ نوسان است"
    انشعابِ هاپف باعث به‌وجود آمدنِ یک چرخهٔ حدی با **بسامدِ غیرصفر** می‌شود — این در نورون‌ها به تحریک‌پذیریِ **نوعِ ۲** می‌انجامد. اما راهِ دیگری هم هست: **انشعابِ زین–گره روی دایرهٔ ناوردا (SNIC)**، که در آن بسامد **پیوسته از صفر** آغاز می‌شود (**نوعِ ۱**). این دو راه و منحنیِ بسامد–جریانِ متمایزشان را در فصلِ [تحریک‌پذیری](ch-dynamics-07-neuro-excitability.md) می‌بینیم.

    این تمایز صرفاً ریاضی نیست: هر دو راه در مغز پیاده شده‌اند و اندازه‌گیری شده‌اند. **سلول‌های هرمیِ قشر** از راهِ SNIC روشن می‌شوند و می‌توانند با چند هرتز شلیک کنند؛ **اینترنورون‌های مهاریِ سریع‌شلیکِ PV+** و آکسونِ غول‌پیکرِ اسکویید از راهِ هاپف روشن می‌شوند و کمینه‌بسامدِ غیرصفر دارند. یعنی وقتی می‌گوییم «یک انشعاب کدام است»، داریم دربارهٔ یک واقعیتِ زیستیِ قابلِ سنجش حرف می‌زنیم، نه دربارهٔ یک برچسبِ صوری.
...
## قضیهٔ پوانکاره–بندیکسون: چرا آشوب در دو بُعد ممکن نیست
...
پیش از گشودنِ پاسخ، خودتان شبیه‌سازی را اجرا کنید و رفتارِ چرخهٔ حدی را ببینید.

!!! question "تمرینِ ۱"
    فرمِ نرمالِ هاپف را برای چند \(\mu>0\) شبیه‌سازی کنید و تأیید کنید که شعاعِ نهاییِ چرخهٔ حدی برابرِ \(\sqrt{\mu}\) است.

??? success "پاسخ"
    از همان تابعِ `hopf` این فصل انتگرال می‌گیریم و برای هر \(\mu>0\) شعاعِ نهاییِ مسیر، \(\sqrt{x^2+z^2}\)، را اندازه می‌گیریم. طبق فرمِ نرمالِ قطبیِ \(\dot r=\mu r-r^3\)، تعادلِ ناصفرِ شعاع در \(r^*=\sqrt{\mu}\) است، پس انتظار داریم مسیر روی دایره‌ای به همین شعاع بنشیند.
    ...
    for mu in (0.25, 0.5, 1.0, 2.0):
        traj = scipy.integrate.odeint(partial(hopf, mu=mu), [0.05, 0.05], t)
        print(f"mu={mu}  r_final={np.hypot(*traj[-1]):.4f}  sqrt(mu)={np.sqrt(mu):.4f}")
    ```

    خروجی برای \(\mu=0.25,0.5,1,2\) به‌ترتیب شعاعِ \(0.5000,\ 0.7071,\ 1.0000,\ 1.4142\) می‌دهد که دقیقاً \(\sqrt{\mu}\) است؛ دامنهٔ نوسان مانندِ \(\sqrt{\mu}\) رشد می‌کند.

!!! question "تمرینِ ۲"
    **** نوسانگرِ فان‌در‌پل را از یک نقطهٔ درونی و یک نقطهٔ بیرونی شبیه‌سازی کنید و نشان دهید هر دو به ** همان** چرخهٔ حدی می‌رسند — ویژگیِ تعریف‌کنندهٔ چرخهٔ حدی.

??? success "پاسخ"
    همان تابعِ `vdp` این فصل را با \(\mu=5\) از یک شرطِ اولیهٔ نزدیکِ مبدأ (درون) و یک شرطِ اولیهٔ دور (بیرون) انتگرال می‌گیریم. چون چرخهٔ حدی **منزوی و جذب‌کننده** است، هر دو گذرا باید روی یک منحنیِ بسته و با یک دامنهٔ یکسان بنشینند.
    ...
    # دامنهٔ x روی بخشِ پایانی (پس از میرایی گذراها)
    print(np.max(np.abs(inside[-5000:, 0])),
          np.max(np.abs(outside[-5000:, 0])))     # هر دو ~ 2.02
    ```

    هر دو مسیر به بیشینهٔ دامنهٔ \(\approx 2.02\) می‌رسند: صرف‌نظر از آغازِ درونی یا بیرونی، دستگاه روی همان چرخهٔ حدیِ یگانه می‌نشیند.

!!! question "تمرینِ ۳"
    دورهٔ نوسانِ فان‌در‌پل را بر حسبِ \(\mu\) برای \(\mu \in [0.1, 10]\) اندازه بگیرید و رشدِ آن را با بزرگ‌شدنِ \(\mu\) (رژیمِ واهلشی) نشان دهید.

??? success "پاسخ"
    برای هر \(\mu\) پس از عبور از گذرا، دوره را از فاصلهٔ میانگینِ عبورهای صعودیِ \(x\) از صفر می‌سنجیم. برای \(\mu\) کوچک نوسان تقریباً سینوسی است و دوره نزدیکِ \(2\pi\approx 6.28\) می‌ماند؛ با بزرگ‌شدنِ \(\mu\) و ورود به رژیمِ واهلشی، دوره تقریباً خطی با \(\mu\) رشد می‌کند (به‌طورِ مجانبی \(T\approx(3-2\ln 2)\,\mu\)).

    ```python
    def vdp_period(mu):
        t = np.linspace(0, 200, 40000)
        x = scipy.integrate.odeint(partial(vdp, mu=mu), [2.0, 0.0], t)[:, 0]
        x, ts = x[len(x)//2:], t[len(t)//2:]          # نیمهٔ دومِ پایدار
        up = np.where((x[:-1] < 0) & (x[1:] >= 0))[0]  # عبورهای صعودی از صفر
        return np.mean(np.diff(ts[up]))

    for mu in (0.1, 1, 2, 5, 10):
        print(f"mu={mu:<4} period={vdp_period(mu):.2f}")
    ```

    دوره‌های اندازه‌گیری‌شده تقریباً \(6.29,\ 6.66,\ 7.63,\ 11.61,\ 19.08\) هستند: از نزدیکیِ \(2\pi\) در \(\mu=0.1\) آغاز می‌شوند و با بزرگ‌ترشدنِ \(\mu\) به‌روشنی رشد می‌کنند — امضای زمانیِ نوسانِ واهلشی.
