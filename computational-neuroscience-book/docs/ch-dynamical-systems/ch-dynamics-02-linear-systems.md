...
پیش از خواندنِ پاسخ، خودتان مقادیرِ ویژه را روی کاغذ محاسبه و سپس با کد بسنجید.

!!! question "تمرینِ ۱"
    یک ماتریسِ \(2\times2\) دلخواه بسازید، مقادیرِ ویژه‌اش را با دست محاسبه کنید، مبدأ را رده‌بندی کنید، سپس با `np.linalg.eig` و با شبیه‌سازیِ \(\dot{\mathbf u}=A\mathbf u\) از چند شرطِ اولیه بسنجید.

??? success "پاسخ"
    ماتریسِ \(A=\begin{bmatrix} 1 & 2 \\ 3 & 2 \end{bmatrix}\) را بگیرید. معادلهٔ مشخصه \(\det(A-\lambda I)=(1-\lambda)(2-\lambda)-6=\lambda^2-3\lambda-4=0\) است، پس \(\lambda_{1,2}=\frac{3\pm\sqrt{9+16}}{2}=4,\,-1\). دو مقدارِ ویژهٔ حقیقی با علامتِ مخالف یعنی مبدأ یک **زین** (ناپایدار) است. کد این را تأیید می‌کند و شبیه‌سازی نشان می‌دهد جز مسیرهایی که دقیقاً روی راستایِ ویژهٔ پایدار (متناظرِ \(\lambda=-1\)) آغاز شوند، بقیه واگرا می‌شوند.
    ...
    A = np.array([[1., 2.], [3., 2.]])
    print(np.linalg.eig(A)[0])            # [-1.  4.]  -> زین

    def lin(u, t, A): return A @ u
    t = np.linspace(0, 3, 300)
    for u0 in ([1, 0], [0, 1], [0.01, -0.01]):
        end = scipy.integrate.odeint(lin, u0, t, args=(A,))[-1]
        print(u0, "->", end)
    ```

    مقادیرِ ویژهٔ عددی \(-1\) و \(4\) دقیقاً با محاسبهٔ دستی می‌خوانند و رفتارِ واگرای شبیه‌سازی، رده‌بندیِ «زین» را تأیید می‌کند.

!!! question "تمرینِ ۲"
    تمرینِ ۱ را تنها با \(\operatorname{Tr}(A)\) و \(\det(A)\) تکرار کنید — بدونِ محاسبهٔ مقادیرِ ویژه. به همان نتیجه می‌رسید؟

??? success "پاسخ"
    برای همان \(A\) داریم \(\operatorname{Tr}(A)=1+2=3\) و \(\det(A)=1\cdot2-2\cdot3=-4\). چون \(D<0\)، جدولِ اثر–دترمینانِ همین فصل بی‌درنگ «زین» می‌دهد — بدونِ هیچ محاسبهٔ مقدارِ ویژه‌ای. این دقیقاً همان نتیجهٔ تمرینِ ۱ است.

    ```python
    T, D = np.trace(A), np.linalg.det(A)
    print(T, D, T**2 - 4*D)     # 3.0  -4.0  25.0
    # D < 0  ->  زین  (همان پاسخِ تمرینِ ۱)
    ```

    نکته این است که علامتِ \(\det(A)\) به‌تنهایی برای تشخیصِ زین کافی است؛ اثر و مبیّن تنها هنگامی که \(D>0\) باشد نقشِ تفکیک‌کننده می‌یابند.

!!! question "تمرینِ ۳"
    برای ماتریسِ \(A=\begin{bmatrix} 0 & 1 \\ -1 & -c \end{bmatrix}\)، نشان دهید با تغییرِ \(c\) از منفی به مثبت، مبدأ از مارپیچِ ناپایدار به مارپیچِ پایدار می‌گذرد. این عبور (در \(c=0\)) همان جایی است که در فصلِ نوسانگرها انشعابِ هاپف را خواهیم دید.

??? success "پاسخ"
    اینجا \(\operatorname{Tr}(A)=-c\) و \(\det(A)=1>0\)، و مقادیرِ ویژه \(\lambda_{1,2}=\frac{-c\pm\sqrt{c^2-4}}{2}\) هستند. برای \(|c|<2\) مبیّن منفی است، پس مقادیرِ ویژه مختلط‌اند و مبدأ یک **مارپیچ** است؛ بخشِ حقیقیِ آن‌ها \(-c/2\) است. پس برای \(c<0\) بخشِ حقیقی مثبت است (مارپیچِ **ناپایدار**) و برای \(c>0\) منفی است (مارپیچِ **پایدار**). درست در \(c=0\) مقادیرِ ویژه \(\pm i\) و مبدأ یک مرکز است — همان عبورِ بحرانیِ انشعابِ هاپف.
    ...
    for c in (-0.5, 0.0, 0.5):
        ...
        print(f"c={c:+.1f}  eig={ev}  Re={ev.real}")
    # c=-0.5 -> Re=+0.25 مارپیچِ ناپایدار
    # c= 0.0 -> Re= 0.00 مرکز
    # c=+0.5 -> Re=-0.25 مارپیچِ پایدار
    ```

    بخشِ حقیقیِ مقادیرِ ویژه با علامتِ \(-c/2\) عوض می‌شود؛ حرکتِ نقطهٔ \((T,D)=(-c,1)\) از راستِ محورِ \(T=0\) به چپِ آن، همان گذرِ هاپف روی صفحهٔ اثر–دترمینان است.
