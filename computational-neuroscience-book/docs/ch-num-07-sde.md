...
نتیجهٔ شبیه‌سازی (واریانسِ نزدیک به $N$ و شکلِ زنگوله‌ای) دقیقاً با پیش‌بینیِ تحلیلی می‌خواند. این توافق، هم اعتمادِ ما به شبیه‌سازی را بالا می‌برد و هم نشان می‌دهد که چرا توزیعِ نرمال در دلِ نوفهٔ تصادفی این‌قدر فراگیر است.

دو شکلِ بالا در واقع دو نمای متفاوت از یک چیزِ واحدند. اگر شمارِ مسیرها را به‌جای پنج، به چند صد برسانیم و مکانِ نهاییِ همهٔ آن‌ها را در لبهٔ راستِ همان نمودار بشماریم، هیستوگرام خودبه‌خود پدیدار می‌شود: توزیعِ مکانِ نهایی چیزی جز **برشِ آخرِ همین دسته مسیرها** نیست.

<figure markdown="span">
  ![Image title](random_walk_marginal.png)
  <figcaption markdown="span">پیوندِ میانِ مسیرها و توزیع. چپ: دستهٔ مسیرهای گشتِ تصادفی (۱۰۰۰۰ گردشگر، که ۲۵۰ مسیرِ آن رسم شده و پنج مسیر برای دنبال‌کردن پررنگ شده‌اند)، به‌همراه پوشِ $\pm\sqrt{t}$ و $\pm 2\sqrt{t}$. راست: هیستوگرامِ مکانِ نهاییِ همان گردشگرها در آخرین گام (خطِ قرمز)، که همان توزیعِ دوجمله‌ای و تقریبِ گاوسی را بازمی‌سازد. چون دو نمودار محورِ عمودیِ مشترک دارند، دیده می‌شود که پهنای زنگوله در سمتِ راست دقیقاً همان‌جایی است که پوشِ $\sqrt{t}$ در سمتِ چپ به آن رسیده است.</figcaption>
</figure>

حال که شهودِ گسسته را ساختیم، به نسخهٔ پیوستهٔ آن، یعنی فرایندِ وینر، می‌پردازیم.
...
با $\sigma=0$ نوسان کاملاً منظم و دامنه‌اش ثابت است (در صفحهٔ فاز یک دایرهٔ بسته). با افزایشِ $\sigma$، نوفه پیوسته نوسانگر را از مدارش بیرون می‌راند: مدارِ صفحهٔ فاز به حلقه‌ای پهن و پُرنوفه بدل می‌شود، اما از میان نمی‌رود.

اینجا یک نکتهٔ ظریف هست که اگر به آن توجه نکنیم، شکلِ زیر گیج‌کننده می‌شود. نوفه تنها به معادلهٔ **سرعت** وارد می‌شود. پس $v$ افزایش‌های وینر را مستقیماً می‌گیرد و ناهموار است، اما $x$ انتگرالِ $v$ است و انتگرال‌گیری یک هموارساز است؛ یعنی $x$ یک مرتبه هموارتر از $v$ باقی می‌ماند. عدد این را روشن می‌کند: با $\sigma=0.6$ و $\Delta t = 0.01$، هر تلنگرِ نوفه سرعت را به‌اندازهٔ $\sigma\sqrt{\Delta t} = 0.06$ جابه‌جا می‌کند، اما مکان را تنها به‌اندازهٔ $0.06 \times \Delta t = 0.0006$، که در مقیاسِ نمودار زیرِ یک پیکسل است. به همین دلیل $x(t)$ صاف به‌نظر می‌رسد، در حالی که نوفه واقعاً در آن هست و خود را نه به‌صورتِ لرزشِ تندِ موضعی، بلکه به‌صورتِ **سرگردانیِ آهستهٔ دامنه و فاز** نشان می‌دهد: در همین شبیه‌سازی، بیشینهٔ $|x|$ از $1.000$ در $\sigma=0$ به $1.739$ در $\sigma=0.6$ و $2.581$ در $\sigma=0.9$ می‌رسد. برای همین در شکلِ زیر، در کنارِ $x(t)$، سری زمانیِ $v(t)$ را هم رسم می‌کنیم؛ ناهمواریِ دیده‌شده در صفحهٔ فاز دقیقاً همان ناهمواریِ $v$ است.
...
# one block per noise level, of the form
#     xxxxppp
#     vvvvppp
# so that x(t) and v(t) share a time axis and the phase plane sits beside them
layout = []
for i in range(len(sigmas)):
    if i:
        layout.append(["."] * 7)                  # spacer row between blocks
    layout.append([f"x{i}"] * 4 + [f"p{i}"] * 3)
    layout.append([f"v{i}"] * 4 + [f"p{i}"] * 3)

fig, axd = plt.subplot_mosaic(layout, figsize=(11, 9.5))
...
for i, sigma in enumerate(sigmas):
    ...
    for k in range(n - 1):
        xs[k+1], vs[k+1] = sho_step(xs[k], vs[k], dt, omega, sigma, rng)
    axd[f"x{i}"].plot(times, xs, color="tab:blue", lw=0.8)
    axd[f"x{i}"].set_ylabel(f"sigma = {sigma}\n\nx")
    axd[f"v{i}"].plot(times, vs, color="tab:orange", lw=0.8)   # the rough one
    axd[f"v{i}"].set_ylabel("v")
    axd[f"p{i}"].plot(xs, vs, color="tab:purple", lw=0.5, alpha=0.8)
    axd[f"p{i}"].set_ylabel("v")

axd["x0"].set_title("time series")
axd["p0"].set_title("phase plane")
axd[f"v{len(sigmas)-1}"].set_xlabel("time t")
axd[f"p{len(sigmas)-1}"].set_xlabel("x")
plt.show()
```

<figure markdown="span">
  ![Image title](em_sho.png)
  <figcaption markdown="span">نوسانگر هماهنگ با توان‌های فزایندهٔ نوفه (σ = ۰، ۰٫۳، ۰٫۶، ۰٫۹). هر ردیف یک توانِ نوفه است: بالا سری زمانیِ مکان $x(t)$ (آبی)، پایین سری زمانیِ سرعت $v(t)$ (نارنجی)، و در سمتِ راست همان مسیر در صفحهٔ فاز (بنفش). محورها میان ردیف‌ها مشترک‌اند تا بزرگ‌شدنِ مدار با $\sigma$ دیده شود. توجه کنید که ناهمواری در $v$ آشکار است و در $x$ نه: نوفه به معادلهٔ سرعت وارد می‌شود و $x$، که انتگرالِ $v$ است، یک مرتبه هموارتر می‌ماند. اثرِ نوفه بر $x$ به‌صورتِ تغییرِ آهستهٔ دامنه و فاز از دوره‌ای به دورهٔ بعد دیده می‌شود، نه به‌صورتِ لرزشِ تند.</figcaption>
</figure>
